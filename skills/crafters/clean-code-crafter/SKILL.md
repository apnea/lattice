...
- **Config key**: `paths.clean_code` in `.ai/config.yaml`
- **Template**: Read `./assets/template.md` for the full document structure, default content, and interview guidance comments

## Scope Clarification

This skill defines the *rules of code craftsmanship* -- how individual functions, classes, and modules should be written. It does not define architecture (that is the architecture-crafter) or domain modeling (that is the ddd-crafter). The boundaries:
...
1. Read `.ai/config.yaml` -- does `paths.clean_code` point to a file?
